...
import { useProveedoresListFilters } from "./proveedores-filters";
import { ProveedoresActionsMenu } from "./proveedores-actions-menu";

export function ProveedoresTable() {
  const t = useTranslations("proveedores");
  const tCommon = useTranslations("common");
  const { filters: urlFilters } = useProveedoresListFilters();
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <Link
            href={`/proveedores/${encodeURIComponent(row.original.code)}`}
            className="font-mono text-xs font-semibold text-primary hover:underline"
            data-testid={`proveedor-row-${row.original.code}`}
          >
            {row.original.code}
          </Link>
        ),
      ...
      {
        id: "paymentTerms",
        header: () => <span>{t("columns.paymentTerms")}</span>,
      ...
      },
      {
        ...
        cell: ({ row }) => <ProveedoresActionsMenu supplier={row.original} />,
      },
    ],
    [t],
  );
  ...
  if (isLoading) {
    return (
      <div className="space-y-2" data-testid="proveedores-loading">
  ...
  if (isError) {
    return (
      <Card data-testid="proveedores-error">
  ...
  if (items.length === 0) {
    return (
      <Card data-testid="proveedores-empty">
  ...
  return (
    <div className="space-y-3" data-testid="proveedores-table-root">
      <DataTable<Supplier, unknown> columns={columns} data={items} />
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span data-testid="proveedores-total-count">
        ...
        {hasNextPage ? (
          <Button
            ...
            data-testid="proveedores-load-more"
          >
