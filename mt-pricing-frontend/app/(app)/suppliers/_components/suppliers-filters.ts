...
/**
 * Filtros de la lista `/suppliers` con state-in-URL.
 * Alineado con backend real: `contract_currency` (no `country`/`currency` legacy).
 */
...
export type SuppliersFilterKey = "q" | "contract_currency" | "active";

export function useSuppliersListFilters() {
  const sp = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();

  const filters: SuppliersListFilters = useMemo(() => {
  ...
  const setFilter = useCallback(
    (key: SuppliersFilterKey, value: string | undefined) => {
