...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "proposed_change", JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
        ...
        sa.CheckConstraint(
            "status IN ('pending','applied','dismissed')", name="ck_rule_suggestion_status"
        ),
    )
    op.create_index(
        "idx_rule_suggestions_profile_status", "rule_suggestions", ["taxonomy_profile_id", "status"]
    )
