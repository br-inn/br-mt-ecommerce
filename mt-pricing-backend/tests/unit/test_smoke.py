...
async def test_liveness_returns_200(async_client) -> None:  # noqa: ANN001
...
async def test_docs_enabled_in_dev(async_client) -> None:  # noqa: ANN001
    resp = await async_client.get("/docs")
    # 200 si ENABLE_DOCS, 404 si no.
    assert resp.status_code in (200, 404)


async def test_celery_eager_health_ping(celery_app_eager) -> None:  # noqa: ANN001
